...
private:
	...
	MemoryManager(std::size_t masterBlockSize) :masterBlock(nullptr) {
		...
		blockIterator = 0;
		int byte16ListSize = ((masterBlockSize / 2) / 16);
		//for (int )
...
public:
	...
	void* allocateToMasterBlock(std::size_t memorySize) {
		void* preIterate = &masterBlock[blockIterator];
		blockIterator += memorySize;
		return preIterate;
		//return &masterBlock[blockIterator += memorySize];
...
struct Vec3 {
	...
	{
		return MemoryManager::GetInstance()->allocate(sizeof(size));
	}
	inline void  operator delete(void* object)
	{
		//MemoryManager::GetInstance()->deallocate(object);
	}
};
